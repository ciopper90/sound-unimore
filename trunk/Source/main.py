...
for i in range(len(v)):
	...
	for k in range(len(v[i])):
		...
		tmp_list = []
                for h in f.values():
			tmp_list.append(str(h))
		tmp_list.append(d[i][0][:string.find(d[i][0],"_")])
                print ','.join(tmp_list)

		#print str(h)+","#file.write(str(h)+",")
		#print d[i][0][:string.find(d[i][0],"_")]+","+d[i][0]+"_"+str(k)+"\n"
